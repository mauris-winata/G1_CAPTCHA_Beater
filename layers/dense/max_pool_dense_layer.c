...
void max_pool_dense_layer(weights_biases_t* mem, // global pointer to weights and biases
	...
	result_t* output, // pointer to the output buffer for resulting data
		...
		for (int o_x = 0; o_x < ox; o_x++)
		{
			...
			for (int i_d = 0; i_d < id; i_d++)
			{
				...
				for (int i_y = 0; i_y < iy; i_y++)
				{
					...
					{
						result_t input_val = input[b_*i_d*iy*ix + i_d*iy*ix + i_y*ix + i_x];
						weights_biases_t weight = mem[offset / sizeof(weights_biases_t) + (i_y*ix*id + i_x*id + i_d)*ox + o_x];
