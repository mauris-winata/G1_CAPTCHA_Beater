...
#define b CONV1_BATCHES
#define od CONV1_NUM_FILTERS
#define ox CONV1_OUTPUT_X
#define oy CONV1_OUTPUT_Y
#define id CONV1_NUM_INPUTS
#define ix CONV1_INPUT_X
#define iy CONV1_INPUT_Y
#define s CONV1_STRIDE
#define k CONV1_KERNEL


void conv_layer1(//result_t * input, 		// where to get inputs
  ...
  for (int b_=0; b_< b; b_++)
  {
    ...
    for (int o_d = 0; o_d < od; o_d++)
    {
      ...
      for (int o_y = 0; o_y < oy; o_y++)
      {
        ...
        {
          // Set bias
        	weights_biases_t bias = conv_weights1[offset/sizeof(weights_biases_t) + num_weights + o_d]; //one bias per filter
          ...
          for (int i_d = 0; i_d < id; i_d++)
          {
            ...
            for (int i_y = o_y*s-k/2, iiy = 0; iiy < k; i_y++, iiy++)
            {
              ...
              for (int i_x = o_x*s-k/2, iix = 0; iix < k; i_x++, iix++)
              {
                ...
                weights_biases_t weight = conv_weights1[offset/sizeof(weights_biases_t) + (k*iiy + iix)*od*id +i_d*od + o_d];
