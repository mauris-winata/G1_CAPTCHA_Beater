...
#define b CONV3_BATCHES
#define od CONV3_NUM_FILTERS
#define ox CONV3_OUTPUT_X
#define oy CONV3_OUTPUT_Y
#define id CONV3_NUM_INPUTS
#define ix CONV3_INPUT_X
#define iy CONV3_INPUT_Y
#define s CONV3_STRIDE
#define k CONV3_KERNEL


void conv_layer3(//result_t * input, 		// where to get inputs
  ...
  for (int b_=0; b_< b; b_++)
  {
    ...
    for (int o_d = 0; o_d < od; o_d++)
    {
      ...
      for (int o_y = 0; o_y < oy; o_y++)
      {
        ...
        {
          // Set bias
        	weights_biases_t bias = conv_weights3[offset/sizeof(weights_biases_t) + num_weights + o_d]; //one bias per filter
          ...
          for (int i_d = 0; i_d < id; i_d++)
          {
            ...
            for (int i_y = o_y*s-k/2, iiy = 0; iiy < k; i_y++, iiy++)
            {
              ...
              for (int i_x = o_x*s-k/2, iix = 0; iix < k; i_x++, iix++)
              {
                ...
                weights_biases_t weight = conv_weights3[offset/sizeof(weights_biases_t) + (k*iiy + iix)*od*id +i_d*od + o_d];
