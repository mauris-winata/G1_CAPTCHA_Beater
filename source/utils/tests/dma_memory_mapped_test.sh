...
if [ $h2cChannels -gt 0 ]; then
  ...
  do
    ...
    $tool_path/dma_to_device -d /dev/xdma0_h2c_${curChannel} -f InputImageTest.bin -s $transferSize -a $addrOffset -c $transferCount &
...
if [ $h2cChannels -eq 0 ]; then
  ...
  for ((i=0; i<=3; i++))
  do
    cmp data/output_datafile${i}_4K.bin InputImageTest.bin -n $transferSize
    returnVal=$?
    if [ ! $returnVal == 0 ]; then
